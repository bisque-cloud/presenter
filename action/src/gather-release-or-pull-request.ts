...
const event = process.env.EVENT_PATH
  ? JSON.parse(readFileSync(process.env.EVENT_PATH, "utf8"))
  : {};

// The format contract, pre-fetched so the agent needs no shell to read it.
run("curl", [
  "-fsSL",
  "https://bisque.cloud/api/presentations/spec",
  "-o",
  join(work, "spec.md"),
]);
...
if (source === "release") {
  ...
  if (!tag)
    fail(
      "source is 'release' but this event carries no release. Use 'on: release: types: [published]', pass tag:, or pass source: pull-request.",
    );
  ...
  const rel = ghJson([
    ...
    repo,
    "--json",
    "name,tagName,body,publishedAt,author,url",
  ]);
  ...
  if (prev) {
    ...
    const cmp = ghJson<Compare>([
      "api",
      `repos/${repo}/compare/${prev}...${tag}`,
    ]);
    const commits = (cmp.commits ?? []).map(
      (c) => `- ${c.sha.slice(0, 7)} ${c.commit.message.split("\n")[0]}`,
    );
    const files = (cmp.files ?? []).map(
    ...
    if (sel.skipped.length || sel.cut.length)
      console.log(
        `diff: ${sel.skipped.length} file(s) skipped, ${sel.cut.length} cut; listed in changes.md`,
      );
  } else {
    writeFileSync(
      join(src, "changes.md"),
      "First release; there is no previous tag to compare against.\n",
    );
  ...
  writeFileSync(
    join(work, "TASK.md"),
    readFileSync(join(here, "prompt-release.md"), "utf8").replaceAll(
      "{{REPO}}",
      repo,
    ),
  );
  ...
  const number =
    process.env.PR_INPUT ||
    String(event.pull_request?.number ?? event.issue?.number ?? "");
  if (!number)
    fail(
      "source is 'pull-request' but this event carries no pull request. Pass pull-request: <number> for a manual run.",
    );
  ...
  const pr = ghJson([
    ...
    repo,
    "--json",
    "title,body,author,mergedAt,labels,files,url,number",
  ]);
  writeFileSync(
    join(src, "pr.md"),
    [
  ...
  const prFiles = ghJson<ChangedFile[]>([
    "api",
  ...
  if (sel.skipped.length || sel.cut.length)
    console.log(
      `diff: ${sel.skipped.length} file(s) skipped, ${sel.cut.length} cut; listed in pr.md`,
    );
  let comments = "";
  try {
  ...
  writeFileSync(
    join(work, "TASK.md"),
    readFileSync(join(here, "prompt-pull-request.md"), "utf8").replaceAll(
      "{{REPO}}",
      repo,
    ),
  );
